...
> **구현·배포 완료 (2026-08-30).** 1~4·6단계가 끝났고 <https://seedevk8s.github.io/tetris2/> 에서 동작합니다.
> **남은 것은 5단계(리다이렉트 주소 등록)뿐**이며, 비밀번호 재설정에만 쓰입니다.
...
**게임을 하려면 로그인해야 합니다.** 로그인 전에는 보드가 잠깁니다(사용자 요청).
랭킹은 로그인 없이도 볼 수 있습니다.

그래서 **Supabase 가 멈추면 게임도 할 수 없습니다.** 무료 플랜은 한동안 쓰지 않으면
프로젝트가 일시정지되므로, 그때는 대시보드에서 되살려야 합니다(아래 무료 플랜 절 참고).
...
일시정지가 실제로 겪게 될 부분입니다. **로그인이 필수이므로 프로젝트가 멈추면 게임도 멈춥니다.**
오랜만에 열었는데 로그인이 안 되면 대시보드에서 **Restore** 를 누르세요.

로그인한 뒤의 실패는 다르게 다룹니다 — 점수 전송이나 랭킹 조회가 실패해도 플레이는 계속됩니다.
